...
for doc in cursor:
    ...
    try:
        destination = os.path.join(home_dir, 'data', 'bench1', doc.get('test_run_id'), 'Reprocessed_Output',
                                   f'xosc_{doc.get("scenario_uuid")}')
        if not os.path.exists(destination):
            os.makedirs(destination)
        response = s3_client.download_file(BUCKET_NAME, path, os.path.join(home_dir, path))
        meta_info = {
            'slno': count,
            'path': path,
